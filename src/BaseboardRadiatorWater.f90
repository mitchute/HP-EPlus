...
CONTAINS
  ...
  SUBROUTINE GetHWBaseboardInput
    ...
    !unused0909    USE DataGlobals_HPSimIntegrated,           ONLY: NumOfZones
...
SUBROUTINE InitHWBaseboard(BaseboardNum, ControlledZoneNumSub, FirstHVACIteration)
  ...
  ! USE STATEMENTS:
  USE DataGlobals_HPSimIntegrated,       ONLY: BeginEnvrnFlag, NumofZones
  USE DataLoopNode,      ONLY: Node
...
SUBROUTINE DistributeBBRadGains
  ...
  ! USE STATEMENTS:
  !USE DataGlobals_HPSimIntegrated,       ONLY : ShowContinueError, ShowWarningError, ShowSevereError, &
  !ShowFatalError
  USE General,           ONLY : RoundSigDigits
