...
CONTAINS
  ...
  SUBROUTINE SimDXCoil(CompName,CompOp,FirstHVACIteration,PartLoadRatio,CompIndex,FanOpMode,OnOffAFR, CoilCoolingHeatingPLRRatio, &
    ...
    CASE (CoilDX_HPSim)   !RS: Implementation: Debugging: Setting up a case when HPSim is called
      IF (.NOT. WARMUPFLAG) THEN
        IF (CurrentTime .GE. 12 .AND. CurrentTime .LE. 13) THEN    !RS: Debugging: Hardcoding so only one hour of HPSim is being run (3/10/18)
  ...
  SUBROUTINE GetDXCoils
    ...
    USE DataGlobals_HPSimIntegrated,           ONLY: AnyEnergyManagementSystemInModel, emsCallFromComponentGetInput
...
!RS: Debugging: Hardcoding in a test case for 0.5 mass flow rate (3/7/15)
!Node(AirInletNode)%MassFlowRate=0.7
...
SUBROUTINE CalcHPSimDXCoil(CompOp,FirstHVACIteration,PartLoadRatio,FanOpMode,PerfMode,OnOffAirFlowRatio, &
  ...
  LOGICAL :: ERRORFLAG    !RS: Debugging: UpdateHVACInterface (8/8/14)
