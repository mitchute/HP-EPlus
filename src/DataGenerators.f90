MODULE DataGenerators
  ...
  TYPE FCPowerModuleStruct
    ...
    REAL(r64)                    :: NomEff   = 0.0 ! nominal efficiency
    REAL(r64)                    :: NomPel  = 0.0 ! nominal power rate at rating point
    INTEGER                      :: NumCycles = 0  ! number of start stop cycles
    REAL(r64)                    :: CyclingDegradRat = 0.0 ! rate of degradation from cycles
    REAL(r64)                    :: NumRunHours = 0.0 ! number of hours of operation
    REAL(r64)                    :: OperateDegradRat = 0.0 ! rate of degradation from run time (per hour)
    REAL(r64)                    :: ThreshRunHours  = 0.0 ! number of hours before degradation starts
    REAL(r64)                    :: UpTranLimit = 0.0 !power up transient limit
    REAL(r64)                    :: DownTranLimit = 0.0 !power down tran limit
    REAL(r64)                    :: StartUpTime   = 0.0 !time for start up [hours]
    REAL(r64)                    :: StartUpFuel   = 0.0 !fuel use during start up
    REAL(r64)                    :: StartUpElectConsum  = 0.0 !electricity used during start up
    REAL(r64)                    :: StartUpElectProd  = 0.0 !electricity produced during start up
    REAL(r64)                    :: ShutDownTime  = 0.0 ! time to shut down [hours]
    REAL(r64)                    :: ShutDownFuel  = 0.0 ! fuel consumed during shut down
    REAL(r64)                    :: ShutDownElectConsum = 0.0 !Elect consumed during shut down
    REAL(r64)                    :: ANC0 = 0.0 ! Ancilliary Loads constant term
    REAL(r64)                    :: ANC1 = 0.0 ! Ancilliary Loads linear term
    ...
    REAL(r64)                    :: RadiativeFract = 0.0 !
    REAL(r64)                    :: QdotSkin = 0.0 !
    REAL(r64)                    :: UAskin = 0.0 !
    Integer                      :: SkinLossCurveID = 0 !
    INTEGER                      :: WaterSupplyCurveID = 0 ! pointer to curve for water use in reforming
    REAL(r64)                    :: NdotDilutionAir = 0.0 ! user defined constant flow of dilution air (kmol/sec)
    REAL(r64)                    :: StackHeatLossToDilution = 0.0 ! (watts)
    ...
    REAL(r64)                    :: Eel             = 0.0 ! power module efficiency
    REAL(r64)                    :: QdotStackCool   = 0.0 ! Heat removed by stack cooler
    REAL(r64)                    :: FractionalDayofLastStartUp = 0.0 !fractional days into simulation
    REAL(r64)                    :: FractionalDayofLastShutDown = 0.0 !fractional Days into simulations
    LOGICAL                      :: HasBeenOn =.true.
    LOGICAL                      :: DuringShutDown = .false.
    LOGICAL                      :: DuringStartUp = .false.
    REAL(r64)                    :: NdotFuel = 0.0 ! molar fuel use rate.  (kmol/sec)
    REAL(r64)                    :: TotFuelInEnthalphy = 0.0 ! Enthalpy of fuel coming into FCPM (watts)
    REAL(r64)                    :: NdotProdGas       = 0.0  !(kmol/sec)
    ...
    REAL(r64)                    :: TprodGasLeavingFCPM = 0.0
    REAL(r64)                    :: NdotAir  = 0.0 ! molar air use rate    (kmol/sec)
    REAL(r64)                    :: TotAirInEnthalphy  = 0.0 ! Enthalpy of air coming nto FCPM energy balance (watts)
    REAL(r64)                    :: NdotLiqwater= 0.0    ! molar water use rate (kmol/sec)
    REAL(r64)                    :: TwaterInlet = 0.0    !
    REAL(r64)                    :: WaterInEnthalpy    = 0.0 ! Enthalpy of liquid water used for reforming (watts)
    REAL(r64)                    :: DilutionAirInEnthalpy = 0.0  ! Enthalpy of Dilution air coming into FCPM (watts)
    REAL(r64)                    :: DilutionAirOutEnthalpy = 0.0 !
    REAL(r64)                    :: PelancillariesAC    = 0.0  !ancillary power (watts)
    REAL(r64)                    :: TotProdGasEnthalphy = 0.0 !Enthalphy of product gases leaving FCPM   (watts)
  ...
  TYPE FCAirSupplyDataStruct
    ...
    REAL(r64)                    :: TairIntoBlower = 0.0 ! temperature entering blower
    REAL(r64)                    :: TairIntoFCPM   = 0.0 ! temperature leaving blower and entering FCPM
    REAL(r64)                    :: PairCompEl     = 0.0 ! power drawn by compressor
  ...
  TYPE FCStackCoolerDataStruct
    ...
    REAL(r64)                    :: TstackNom        = 0.0 ! nominal fuel cell stack temperature
    REAL(r64)                    :: TstackActual     = 0.0 ! actual fuel cell stack temperature
    REAL(r64)                    :: r0               = 0.0 ! stack cooling power coefficient r0
    REAL(r64)                    :: r1               = 0.0 ! stack cooling power coefficient r1
    REAL(r64)                    :: r2               = 0.0 ! stack cooling power coefficient r2
    REAL(r64)                    :: r3               = 0.0 ! stack cooling power coefficient r3
    REAL(r64)                    :: MdotStackCoolant = 0.0 ! stack coolant flow rate kg/s
    REAL(r64)                    :: UAs_cool         = 0.0 ! stack heat transfer coef
    REAL(r64)                    :: Fs_cogen         = 0.0 !
    REAL(r64)                    :: As_cogen         = 0.0 !
    REAL(r64)                    :: MdotCogenNom     = 0.0 !
    REAL(r64)                    :: hCogenNom        = 0.0
    REAL(r64)                    :: ns               = 0.0 !
    REAL(r64)                    :: PstackPumpEl     = 0.0
    REAL(r64)                    :: PmpPowerLossFactor = 0.0
    REAL(r64)                    :: f0 = 0.0
    REAL(r64)                    :: f1 = 0.0
    REAL(r64)                    :: f2 = 0.0
  ...
  TYPE FCWaterSupplyDataStruct
    ...
    REAL(r64)                    :: PmpPowerLossFactor = 0.0 !Pump heat loss factor
    !calculated data
    LOGICAL                      :: IsModeled  = .TRUE.
    REAL(r64)                    :: TwaterIntoCompress = 0.0 ! inlet Water Temperature
    REAL(r64)                    :: TwaterIntoFCPM     = 0.0 ! pumped water temp
    REAL(r64)                    :: PwaterCompEl       = 0.0 ! water pump power
    REAL(r64)                    :: QskinLoss          = 0.0 ! pumping losses for zone
  ...
  TYPE FCAuxilHeatDataStruct
    ...
    CHARACTER(len=MaxNameLength) :: ZoneName  = ' '
    INTEGER                      :: ZoneID    = 0
    REAL(r64)                    :: UASkin    = 0.0 !for skin losses to zone
    REAL(r64)                    :: ExcessAirRAT = 0.0 !
    REAL(r64)                    :: ANC0      = 0.0
    REAL(r64)                    :: ANC1      = 0.0
    INTEGER                      :: SkinLossDestination = 0 !control mode for where lost heat goes
    REAL(r64)                    :: MaxPowerW = 0.0
    REAL(r64)                    :: MinPowerW = 0.0
    REAL(r64)                    :: MaxPowerkmolperSec = 0.0
    REAL(r64)                    :: MinPowerkmolperSec = 0.0
    ! calculated and from elsewhere
    INTEGER                      :: NumConstituents = 0
    REAL(r64)                    :: TauxMix           = 0.0
    REAL(r64)                    :: NdotAuxMix        = 0.0
    REAL(r64), DIMENSION(14)           :: ConstitMolalFract = 0.0
    INTEGER, DIMENSION(14)        :: GasLibID          = 0 ! lookup ID in Gas Phase ThermoChemistry Structure Array
    REAL(r64)                    :: QskinLoss         = 0.0 !Heat lost to room
  ...
  TYPE FCExhaustHXDataStruct
    ...
    CHARACTER(len=MaxNameLength) :: WaterOutNodeName = ' ' !HR water outlet Node name
    INTEGER                      :: WaterOutNode     = 0 ! HR Water outlet Node ID
    REAL(r64)                    :: WaterVolumeFlowMax = 0.0 ! HR water flow rate max avail
    ...
    REAL(r64)                    :: HXEffect         = 0 ! Heat Exchanger Effectiveness (method 1)
    REAL(r64)                    :: hxs0             = 0.0 !(method 2)
    REAL(r64)                    :: hxs1             = 0.0 ! (method 2)
    REAL(r64)                    :: hxs2             = 0.0 ! (method 2)
    REAL(r64)                    :: hxs3             = 0.0 ! (method 2)
    REAL(r64)                    :: hxs4             = 0.0 ! (method 2)
    REAL(r64)                    :: h0gas            = 0.0 ! (method 3)
    REAL(r64)                    :: NdotGasRef       = 0.0 ! (method 3)
    REAL(r64)                    :: nCoeff           = 0.0 ! (method 3)
    REAL(r64)                    :: AreaGas          = 0.0 ! (method 3)
    REAL(r64)                    :: h0Water          = 0.0  ! (method 3)
    REAL(r64)                    :: NdotWaterRef     = 0.0 !(method 3)
    REAL(r64)                    :: mCoeff           = 0.0 ! (method 3)
    REAL(r64)                    :: AreaWater        = 0.0 !(method 3)
    REAL(r64)                    :: Fadjust          = 0.0     ! (method 3)
    ...
    REAL(r64)                    :: WaterMassFlowRate  = 0.0 ! water flow rate in plant loop
    REAL(r64)                    :: WaterInletTemp  = 0.0 !
    REAL(r64)                    :: WaterVaporFractExh = 0.0 ! water vapor fraction in exhaust gas stream.
    REAL(r64)                    :: CondensateRate     = 0.0 ! water condensation rate.
    REAL(r64), DIMENSION(14)     :: ConstitMolalFract = 0.0
    INTEGER, DIMENSION(14)       :: GasLibID          = 0 ! lookup ID in Gas Phase ThermoChemistry Structure Array
    REAL(r64)                    :: NdotHXleaving       = 0.0
  ...
  TYPE BatteryDichargeDataStruct
    ...
    REAL(r64)                    :: NumInParallel = 0.0
    REAL(r64)                    :: NominalVoltage = 0.0
    REAL(r64)                    :: LowVoltsDischarged = 0.0 !not used
    ...
    REAL(r64)                    :: k =0.0 !parameter in Manwell McGowan model
  ...
  TYPE FCElecStorageDataStruct
    ...
    REAL(r64)                    :: StartingEnergyStored = 0.0 !joules inside
    REAL(r64)                    :: EnergeticEfficCharge = 0.0 ! for
    REAL(r64)                    :: EnergeticEfficDischarge = 0.0
    REAL(r64)                    :: MaxPowerDraw  = 0.0 ! for simple bucket method 0
    REAL(r64)                    :: MaxPowerStore  = 0.0 ! for simple bucket method 0
    REAL(r64)                    :: NominalVoltage = 0.0 !
    REAL(r64)                    :: NominalEnergyCapacity = 0.0 ! [J]
    !calculated and from elsewhere vars
    REAL(r64)                    :: ThisTimeStepStateOfCharge =0.0 ! [J]
    REAL(r64)                    :: LastTimeStepStateOfCharge =0.0! [J]
    REAL(r64)                    :: PelNeedFromStorage =0.0
    REAL(r64)                    :: IdesiredDischargeCurrent = 0.0 !
    REAL(r64)                    :: PelFromStorage  = 0.0 ! power
    REAL(r64)                    :: IfromStorage = 0.0 ! current this timestepm
    REAL(r64)                    :: PelIntoStorage = 0.0 !
    REAL(r64)                    :: QairIntake =0.0! heat into intake air
    !nested structures
  ...
  TYPE GeneratorFuelSupplyDataStruct
    ...
    REAL(r64)                    :: CompPowerLossFactor = 0.0
    ...
    INTEGER, DIMENSION(14)       :: GasLibID  = 0 ! lookup ID in Gas Phase ThermoChemistry Structure Array
    REAL(r64)                    :: LHV  = 0.0     ! lower heating value of gaseous fuel (kJ/mol)
    REAL(r64)                    :: LHVJperkg = 0.0  ! lower heating value of gaseous fuel (J/kg)
    REAL(r64)                    :: LHVliquid = 0.0 ! userdefined lhv for generic liquid (J/kg)
    REAL(r64)                    :: HHV  = 0.0     ! higher heating value of fuel (J/kg)
    REAL(r64)                    :: MW   = 0.0     ! molecular weight g/mol
    REAL(r64)                    :: eCO2  = 0.0     ! mass flow based CO2 emmissions factor for complete combustion (-)
    REAL(r64)                    :: KmolPerSecToKgPerSec = 0.0 ! conversion from moles to kilograms for this fuel. (
    REAL(r64)                    :: StoicOxygenRate = 0.0
    REAL(r64)                    :: TfuelIntoCompress = 0.0  ! inlet fuel temperature
    REAL(r64)                    :: TfuelIntoFCPM     = 0.0  ! compressed fuel temp
    REAL(r64)                    :: PfuelCompEl       = 0.0  ! fuel compressor power
    REAL(r64)                    :: QskinLoss         = 0.0  ! pumping losses for zone
  ...
  TYPE GasPropertyDataStruct
    ...
    REAL(r64)                    :: ShomateA = 0.0
    REAL(r64)                    :: ShomateB = 0.0
    REAL(r64)                    :: ShomateC = 0.0
    REAL(r64)                    :: ShomateD = 0.0
    REAL(r64)                    :: ShomateE = 0.0
    REAL(r64)                    :: ShomateF = 0.0
    REAL(r64)                    :: ShomateG = 0.0
    REAL(r64)                    :: ShomateH = 0.0
    REAL(r64)                    :: NumCarbons = 0.0
    REAL(r64)                    :: NumHydrogens = 0.0
    REAL(r64)                    :: NumOxygens   = 0.0
    REAL(r64)                    :: MolecularWeight = 0.0
    REAL(r64)                    :: NASA_A1 = 0.0
    REAL(r64)                    :: NASA_A2 = 0.0
    REAL(r64)                    :: NASA_A3 = 0.0
    REAL(r64)                    :: NASA_A4 = 0.0
  ...
  TYPE GeneratorDynamicsManagerStruct
    ...
    REAL(r64)                    :: UpTranLimit = 0.0 !power up transient limit W/s
    REAL(r64)                    :: DownTranLimit = 0.0 !power down tran limit  W/s
    REAL(r64)                    :: UpTranLimitFuel = 0.0 ! fuel up transient limit kg/s
    REAL(r64)                    :: DownTranLimitFuel = 0.0 ! fuel down transient limit kg/s

    LOGICAL                      :: WarmUpByTimeDelay = .false. ! Warm up mode control
    LOGICAL                      :: WarmUpByEngineTemp = .true. ! Warm up mode control
    REAL(r64)                    :: StartUpTimeDelay   = 0.0 !time for start up [hours]
    REAL(r64)                    :: WarmUpDelay   = 0.0 ! time for warm up delay [s]
    ...
    REAL(r64)                    :: ShutDownFuel  = 0.0 ! fuel consumed during shut down
    REAL(r64)                    :: ShutDownElectConsum = 0.0 !Elect consumed during shut down
    REAL(r64)                    :: PcoolDown = 0.0 !power during cool down
    REAL(r64)                    :: CoolDownDelay = 0.0  ! time for cool down delay [hours]

    INTEGER                      :: NumCyclesInit = 0  ! number of start stop cycles at beginning
    REAL(r64)                    :: NumRunHoursInit = 0.0 ! number of hours of operation beginning

    REAL(r64)                    :: Pstandby = 0.0  ! standby power [w]
    REAL(r64)                    :: MCeng = 0.0 ! aggregated thermal mass of engine [  ]
    REAL(r64)                    :: MCcw  = 0.0 ! aggregated thermal mass of heat recovery [   ]
    REAL(r64)                    :: kf = 0.0 ! coefficient k_f for warmup fuel flow rate
    REAL(r64)                    :: TnomEngOp = 0.0 ! nominal engine operating temperature [C]
    REAL(r64)                    :: kp = 0.0 ! coefficient k_p for warmup power
    ...
    REAL(r64)                     :: FractionalDayofLastStartUp = 0.0
    LOGICAL                       :: HasBeenOn = .false.
    LOGICAL                       :: DuringStartUp = .false.
    LOGICAL                       :: DuringShutDown = .false.
    REAL(r64)                     :: FuelMdotLastTimestep  = 0.0
    REAL(r64)                     :: PelLastTimeStep = 0.0
    INTEGER                       :: NumCycles =0
  ...
  Type MicroCHPParamsNonNormalized
    ...
    REAL(r64)                    :: MinElecPower = 0.0 ! net electric power [W]
    REAL(r64)                    :: MinWaterMdot = 0.0 ! minimum cooling water flow [kg/s]
    REAL(r64)                    :: MaxWaterTemp = 0.0 ! limit temp for inlet cooling water [C]
    ...
    REAL(r64)                    :: DeltaPelMax =0.0 ! max rate of change in net electric power [W/s}
    REAL(r64)                    :: DeltaFuelMdotMax = 0.0 !Maximum Rate of change in fuel flow rate [kmol/s2]
    REAL(r64)                    :: UAhx = 0.0 ! heat exchanger UA [W/K]
    REAL(r64)                    :: UAskin = 0.0 ! skin loss UA [W/K]
    REAL(r64)                    :: RadiativeFraction =0.0 ! skin loss fraction to radiant energy []
    REAL(r64)                    :: MCeng = 0.0 ! aggregated thermal mass of engine [J/K]
    REAL(r64)                    :: MCcw  = 0.0 ! aggregated thermal mass of heat recovery [J/k]
    REAL(r64)                    :: Pstandby = 0.0  ! standby power [w]
    LOGICAL                      :: WarmUpByTimeDelay = .false. ! Warm up mode control
    LOGICAL                      :: WarmUpByEngineTemp = .true. ! Warm up mode control
    REAL(r64)                    :: kf = 0.0 ! coefficient k_f for warmup fuel flow rate
    REAL(r64)                    :: TnomEngOp = 0.0 ! nominal engine operating temperature [C]
    REAL(r64)                    :: kp = 0.0 ! coefficient k_p for warmup power
    REAL(r64)                    :: Rfuelwarmup = 0.0 ! Warm Up Fuel Flow Rate Limit Ratio
    REAL(r64)                    :: WarmUpDelay = 0.0 ! time for warm up delay [s]
    REAL(r64)                    :: PcoolDown = 0.0 !power during cool down
    REAL(r64)                    :: CoolDownDelay = 0.0  ! time for cool down delay [s]
    LOGICAL                      :: MandatoryFullCoolDown = .false. !
    LOGICAL                      :: WarmRestartOkay       = .true.
    ! calculated and from elsewhere
    ...
    REAL(r64)                    :: TengLast = 20.0  ! last timestep's value for engine temperature
    REAL(r64)                    :: TempCWOutLast = 20.0  !  last timestep's value for cooling water outlet temperature
    REAL(r64)                    :: Pnet = 0.0
