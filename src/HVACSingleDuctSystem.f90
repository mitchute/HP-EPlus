...
CONTAINS
  ...
  SUBROUTINE GetSysInput
    ...
    USE DataGlobals_HPSimIntegrated,       ONLY: DoZoneSizing
...
SUBROUTINE SizeSys(SysNum)
  ...
  USE DataGlobals_HPSimIntegrated,         ONLY: AutoCalculate
...
SUBROUTINE SimVAV(SysNum,FirstHVACIteration, ZoneNum, ZoneNodeNum)
  ...
  !RS: Debugging: The following two lines were modified to handle full load not remaining (10/22/14)
  !QTotLoad=ZoneSysEnergyDemand(ZoneNum)%TotalOutputRequired * LeakLoadMult
  !QToHeatSetPt=ZoneSysEnergyDemand(ZoneNum)%OutputRequiredToHeatingSP * LeakLoadMult
  !RS: Debugging: End of modifications (10/22/14)
