MODULE Humidifiers
  ...
  USE DataGlobals_HPSimIntegrated, ONLY: MaxNameLength, BeginEnvrnFlag, InitConvTemp, SysSizingCalc, SecInHour !, &
...
SUBROUTINE ControlHumidifier(HumNum,WaterAddNeeded)
  ...
  AirMassFlowRate = 0.0
  UnitOn = .TRUE.
  IF (Humidifier(HumNum)%HumRatSet .LE. 0.0) UnitOn = .FALSE.
...
SUBROUTINE CalcElecSteamHumidifier(HumNum,WaterAddNeeded)
  ...
  ELSE IF (GetCurrentScheduleValue(Humidifier(HumNum)%SchedPtr) .GT. 0.0)  THEN
