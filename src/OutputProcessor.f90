...
USE DataGlobals_HPSimIntegrated, ONLY: MaxNameLength, OutputFileMeters, HourOfDay, DayOfSim, DayOfSimChr, OutputFileStandard, &
                       MinutesPerTimeStep, ZoneTSReporting, HVACTSReporting, StdOutputRecordCount !, ShowSevereError, &
                       !ShowWarningError, ShowFatalError, ShowContinueError
USE DataInterfaces, ONLY: ShowRecurringSevereErrorAtEnd
...
TYPE MeterType
  ...
  CHARACTER(len=5)              :: Units       =BlankString  ! Units for the Meter
...
SUBROUTINE SetupTimePointers(IndexKey,TimeStep)
          ...
          ! USE STATEMENTS:
  USE DataGlobals_HPSimIntegrated, Only: OutputFileStandard !, ShowSevereError
  ...
  DO Loop=MinIndx,MaxIndx
    ...
    IF (.not. Dup) THEN
      ...
      IF (NumExtraVars == NumReportList) THEN
        ALLOCATE(TmpReportList(NumReportList+10))
        TmpReportList=0
        TmpReportList(1:NumReportList)=ReportList
        DEALLOCATE(ReportList)
        NumReportList=NumReportList+10
...
SUBROUTINE GetReportVariableInput
  ...
  NumOfReqVariables=GetNumObjectsFound(TRIM(cCurrentModuleObject))
  ALLOCATE(ReqRepVars(NumOfReqVariables))

  DO Loop=1,NumOfReqVariables

    CALL GetObjectItem(TRIM(cCurrentModuleObject),Loop,cAlphaArgs,NumAlpha,rNumericArgs,NumNumbers,IOStat,  &
...
SUBROUTINE GetCustomMeterInput(ErrorsFound)
  ...
  NumCustomMeters=GetNumObjectsFound(TRIM(cCurrentModuleObject))
  ...
  NumCustomDecMeters=GetNumObjectsFound(TRIM(cCurrentModuleObject))
...
FUNCTION DetermineMeterIPUnits(ResourceType,MtrUnits) RESULT(CodeForIPUnits)
...
!  write(outputfiledebug,*) 'ipunits type=',CodeForIPUnits
  IF (.not. SameString(MtrUnits,'kg') .and. .not. SameString(MtrUnits,'J') .and.   &
    ...
    CALL ShowSevereError('DetermineMeterIPUnits: Meter units not recognized for IP Units conversion='//trim(MtrUnits))
  ENDIF

  RETURN

END FUNCTION DetermineMeterIPUnits
...
SUBROUTINE UpdateMeters(TimeStamp)
  ...
  DO Meter=1,NumEnergyMeters
    IF (EnergyMeters(Meter)%TypeOfMeter /= MeterType_CustomDec .and.   &
      ...
      CALL SetMinMax(MeterValue(Meter),TimeStamp,EnergyMeters(Meter)%HRMaxVal,EnergyMeters(Meter)%HRMaxValDate,  &
      ...
      CALL SetMinMax(MeterValue(Meter),TimeStamp,EnergyMeters(Meter)%DYMaxVal,EnergyMeters(Meter)%DYMaxValDate,  &
      ...
      CALL SetMinMax(MeterValue(Meter),TimeStamp,EnergyMeters(Meter)%MNMaxVal,EnergyMeters(Meter)%MNMaxValDate,  &
      ...
      CALL SetMinMax(MeterValue(Meter),TimeStamp,EnergyMeters(Meter)%SMMaxVal,EnergyMeters(Meter)%SMMaxValDate,  &
  ...
  DO Loop=1,NumEnergyMeters
    SELECT CASE(EnergyMeters(Loop)%RT_forIPUnits)
      CASE(RT_IPUnits_Electricity)
        CALL PreDefTableEntry(pdchEMelecannual,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMValue*convertJtoGJ)
        CALL PreDefTableEntry(pdchEMelecminvalue,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMMinVal/SecondsPerTimeStep)
        CALL PreDefTableEntry(pdchEMelecminvaluetime,EnergyMeters(Loop)%Name,DateToStringWithMonth(EnergyMeters(Loop)%SMMinValDate))
        CALL PreDefTableEntry(pdchEMelecmaxvalue,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMMaxVal/SecondsPerTimeStep)
        CALL PreDefTableEntry(pdchEMelecmaxvaluetime,EnergyMeters(Loop)%Name,DateToStringWithMonth(EnergyMeters(Loop)%SMMaxValDate))
      CASE(RT_IPUnits_Gas)
        CALL PreDefTableEntry(pdchEMgasannual,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMValue*convertJtoGJ)
        CALL PreDefTableEntry(pdchEMgasminvalue,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMMinVal/SecondsPerTimeStep)
        CALL PreDefTableEntry(pdchEMgasminvaluetime,EnergyMeters(Loop)%Name,DateToStringWithMonth(EnergyMeters(Loop)%SMMinValDate))
        CALL PreDefTableEntry(pdchEMgasmaxvalue,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMMaxVal/SecondsPerTimeStep)
        CALL PreDefTableEntry(pdchEMgasmaxvaluetime,EnergyMeters(Loop)%Name,DateToStringWithMonth(EnergyMeters(Loop)%SMMaxValDate))
      CASE(RT_IPUnits_Cooling)
        CALL PreDefTableEntry(pdchEMcoolannual,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMValue*convertJtoGJ)
        CALL PreDefTableEntry(pdchEMcoolminvalue,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMMinVal/SecondsPerTimeStep)
        CALL PreDefTableEntry(pdchEMcoolminvaluetime,EnergyMeters(Loop)%Name,DateToStringWithMonth(EnergyMeters(Loop)%SMMinValDate))
        CALL PreDefTableEntry(pdchEMcoolmaxvalue,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMMaxVal/SecondsPerTimeStep)
        CALL PreDefTableEntry(pdchEMcoolmaxvaluetime,EnergyMeters(Loop)%Name,DateToStringWithMonth(EnergyMeters(Loop)%SMMaxValDate))
      CASE(RT_IPUnits_Water)
        CALL PreDefTableEntry(pdchEMwaterannual,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMValue)
        CALL PreDefTableEntry(pdchEMwaterminvalue,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMMinVal/SecondsPerTimeStep)
        CALL PreDefTableEntry(pdchEMwaterminvaluetime,EnergyMeters(Loop)%Name,  &
           DateToStringWithMonth(EnergyMeters(Loop)%SMMinValDate))
        CALL PreDefTableEntry(pdchEMwatermaxvalue,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMMaxVal/SecondsPerTimeStep)
        CALL PreDefTableEntry(pdchEMwatermaxvaluetime,EnergyMeters(Loop)%Name,  &
           DateToStringWithMonth(EnergyMeters(Loop)%SMMaxValDate))
      CASE(RT_IPUnits_OtherKG)
        CALL PreDefTableEntry(pdchEMotherKGannual,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMValue)
        CALL PreDefTableEntry(pdchEMotherKGminvalue,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMMinVal/SecondsPerTimeStep,3)
        CALL PreDefTableEntry(pdchEMotherKGminvaluetime,EnergyMeters(Loop)%Name,  &
           DateToStringWithMonth(EnergyMeters(Loop)%SMMinValDate))
        CALL PreDefTableEntry(pdchEMotherKGmaxvalue,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMMaxVal/SecondsPerTimeStep,3)
        CALL PreDefTableEntry(pdchEMotherKGmaxvaluetime,EnergyMeters(Loop)%Name,  &
           DateToStringWithMonth(EnergyMeters(Loop)%SMMaxValDate))
      CASE(RT_IPUnits_OtherM3)
        CALL PreDefTableEntry(pdchEMotherM3annual,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMValue,3)
        CALL PreDefTableEntry(pdchEMotherM3minvalue,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMMinVal/SecondsPerTimeStep,3)
        CALL PreDefTableEntry(pdchEMotherM3minvaluetime,EnergyMeters(Loop)%Name,  &
           DateToStringWithMonth(EnergyMeters(Loop)%SMMinValDate))
        CALL PreDefTableEntry(pdchEMotherM3maxvalue,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMMaxVal/SecondsPerTimeStep,3)
        CALL PreDefTableEntry(pdchEMotherM3maxvaluetime,EnergyMeters(Loop)%Name,  &
           DateToStringWithMonth(EnergyMeters(Loop)%SMMaxValDate))
      CASE(RT_IPUnits_OtherL)
        CALL PreDefTableEntry(pdchEMotherLannual,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMValue,3)
        CALL PreDefTableEntry(pdchEMotherLminvalue,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMMinVal/SecondsPerTimeStep,3)
        CALL PreDefTableEntry(pdchEMotherLminvaluetime,EnergyMeters(Loop)%Name,  &
           DateToStringWithMonth(EnergyMeters(Loop)%SMMinValDate))
        CALL PreDefTableEntry(pdchEMotherLmaxvalue,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMMaxVal/SecondsPerTimeStep,3)
        CALL PreDefTableEntry(pdchEMotherLmaxvaluetime,EnergyMeters(Loop)%Name,  &
           DateToStringWithMonth(EnergyMeters(Loop)%SMMaxValDate))
      CASE DEFAULT
        CALL PreDefTableEntry(pdchEMotherJannual,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMValue*convertJtoGJ)
        CALL PreDefTableEntry(pdchEMotherJminvalue,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMMinVal/SecondsPerTimeStep)
        CALL PreDefTableEntry(pdchEMotherJminvaluetime,EnergyMeters(Loop)%Name,  &
           DateToStringWithMonth(EnergyMeters(Loop)%SMMinValDate))
        CALL PreDefTableEntry(pdchEMotherJmaxvalue,EnergyMeters(Loop)%Name,EnergyMeters(Loop)%SMMaxVal/SecondsPerTimeStep)
...
SUBROUTINE WriteReportVariableDictionaryItem (reportingInterval, storeType, reportID, indexGroupKey, &
           indexGroup, reportIDChr, keyedValue, variableName, variableNameWithUnits, indexType, &
           unitsString, scheduleName)
    ...
    ! USE STATEMENTS:
    USE DataGlobals_HPSimIntegrated, ONLY: OutputFileStandard
    USE SQLiteProcedures
    ...
    CHARACTER(len=*), INTENT(IN)           :: variableNameWithUnits ! The variable's actual name plus its units
    INTEGER, INTENT(IN)                    :: indexType
    ...
    SELECT CASE (reportingInterval)
        ...
        CASE (ReportEach, ReportTimeStep)
          WRITE(OutputFileStandard, fmta) TRIM(reportIDChr)//',1,'//  &
             TRIM(keyedValue)//','//TRIM(variableNameWithUnits)//TRIM(FreqString)
...
SUBROUTINE WriteMeterDictionaryItem (reportingInterval, storeType, reportID, indexGroupKey, &
    ...
    ! USE STATEMENTS:
    USE DataGlobals_HPSimIntegrated, ONLY: OutputFileMeters, OutputFileStandard
    USE SQLiteProcedures
...
SUBROUTINE WriteCumulativeReportMeterData (reportID, creportID, timeIndex, repValue, meterOnlyFlag)
    ...
    ! USE STATEMENTS:
    USE DataGlobals_HPSimIntegrated, ONLY: OutputFileMeters, OutputFileStandard, StdOutputRecordCount, StdMeterRecordCount
    USE General, ONLY: RemoveTrailingZeros
    USE SQLiteProcedures
...
SUBROUTINE WriteReportMeterData (reportID, creportID, timeIndex, repValue, reportingInterval, &
    ...
    ! USE STATEMENTS:
    USE DataPrecisionGlobals
    USE DataGlobals_HPSimIntegrated, ONLY: OutputFileStandard, OutputFileMeters, StdOutputRecordCount, StdMeterRecordCount
    USE General, ONLY: RemoveTrailingZeros
    USE SQLiteProcedures
...
SUBROUTINE WriteReportIntegerData (reportID, reportIDString, timeIndex, repValue, storeType,   &
    ...
    INTEGER, INTENT(IN), OPTIONAL :: minValue           ! The variable's minimum value during the reporting interval
    INTEGER, INTENT(IN), OPTIONAL :: minValueDate       ! The date the minimum value occurred
...
!! Errors are severe and fatal because should only be encountered during development.
  ...
  IF (Item /= 0) THEN
    ...
    IF (LEN_TRIM(ADJUSTL(VariableName(1:Item-1))) > MaxNameLength) THEN
      CALL ShowSevereError('Variable Name length exceeds maximum='//TRIM(VariableName(1:Item-1)))
    ...
    IF (RVariable%Report) THEN
      ...
      IF (RVariable%SchedPtr /= 0) THEN
        CALL WriteReportVariableDictionaryItem (RVariable%ReportFreq, RVariable%StoreType, &
            ...
            VariableName, RVariableTypes(CV)%IndexType, RVariableTypes(CV)%UnitsString, &
...
SUBROUTINE FlushHPOutput()
    ...
    REAL tAoCoilEvap, hAoCoilEvap, hAiCoilEvap, tAiCoilEvap
    ...
    IF (CurTime .NE. PrevTime) THEN    !RS: Debugging: Commenting out just to see what other results we get
        ...
        CALL GetEvapProp(tAiCoilEvap, hAiCoilEvap, tAoCoilEvap, hAoCoilEvap)
        ...
        WRITE(OutputFile, '(15(F12.5,","))') CurTime, QSens, QLat, pRiCoil, tRiCoil, hRiCoil, pRoCoil,  &
                                            tRoCoil, hRoCoil,tAoCoil, rhAoCoil, tAiCoilEvap, hAiCoilEvap, tAoCoilEvap, hAoCoilEvap
        PrevTime=CurTime
    END IF
    ...
    IF (IVariable%Report) THEN
      ...
      IF (IVariable%SchedPtr /= 0) THEN
        CALL WriteReportVariableDictionaryItem (IVariable%ReportFreq, IVariable%StoreType, &
             ...
             KeyedValue, VarName, VariableName, IVariableTypes(CV)%IndexType, &
             IVariableTypes(CV)%UnitsString, ReqRepVars(ReportList(Loop))%SchedName)
      ELSE
        CALL WriteReportVariableDictionaryItem (IVariable%ReportFreq, IVariable%StoreType, &
             IVariable%ReportID, localIndexGroupKey, IndexTypeKey, IVariable%ReportIDChr, &
             KeyedValue, VarName, VariableName, IVariableTypes(CV)%IndexType, &
             IVariableTypes(CV)%UnitsString)
      END IF
    ENDIF
  ENDDO
...
SUBROUTINE SetupRealOutputVariable_IntKey(VariableName,ActualVariable,IndexTypeKey,VariableTypeKey,KeyedValue,  &
  ...
  IF (PRESENT(indexGroupKey)) THEN
      CALL SetupOutputVariable(VariableName,ActualVariable,IndexTypeKey,VariableTypeKey,IDOut,  &
...
SUBROUTINE UpdateDataandReport(IndexTypeKey)
  ...
  USE DataGlobals_HPSimIntegrated, ONLY: HourOfDay, DayOfSimChr, EndHourFlag, EndDayFlag, EndEnvrnFlag, OutputFileStandard
  ...
  ! Basic record keeping and report out if "detailed"
    ...
    TimeValue(IndexType)%CurMinute=TimeValue(IndexType)%CurMinute+TimeValue(IndexType)%TimeStep*60.
...
!        IF (ReportNow) THEN
        IF (RVar%tsStored) THEN
          ...
          RVar%NumStored=RVar%NumStored+1.0
        ENDIF
        RVar%tsStored=.false.
        RVar%thisTSstored=.false.
        RVar%thisTScount=0
...
!        IF (ReportNow) THEN
        IF (IVar%tsStored) THEN
          ...
          IVar%NumStored=IVar%NumStored+1.0
        ENDIF
        IVar%tsStored=.false.
        IVar%thisTSstored=.false.
        IVar%thisTScount=0
...
SUBROUTINE GenOutputVariablesAuditReport
  ...
  USE DataPrecisionGlobals
  USE OutputProcessor
  USE DataGlobals_HPSimIntegrated,    ONLY: DisplayAdvancedReportVariables !, ShowMessage
...
SUBROUTINE UpdateMeterReporting
  ...
  NumReqMeters=GetNumObjectsFound(TRIM(cCurrentModuleObject))

  DO Loop=1,NumReqMeters

    CALL GetObjectItem(TRIM(cCurrentModuleObject),Loop,Alphas,NumAlpha,Numbers,NumNumbers,IOStat,  &
                   AlphaBlank=lAlphaFieldBlanks,NumBlank=lNumericFieldBlanks,  &
                   AlphaFieldnames=cAlphaFieldNames,NumericFieldNames=cNumericFieldNames)
  ...
  NumReqMeterFOs=GetNumObjectsFound(TRIM(cCurrentModuleObject))
  ...
  CASE (-1:0)  ! roll "detailed" into TimeStep
    IF (.not. CumulativeIndicator) THEN
      ...
      IF (.not. EnergyMeters(WhichMeter)%RptTS) THEN
        ...
        CALL WriteMeterDictionaryItem (FrequencyIndicator, 1, EnergyMeters(WhichMeter)%TSRptNum, &
    ...
    ELSE
      ...
      IF (.not. EnergyMeters(WhichMeter)%RptAccTS) THEN
        ...
        indexGroupKey = DetermineIndexGroupKeyFromMeterName (EnergyMeters(WhichMeter)%Name)
        indexGroup    = DetermineIndexGroupFromMeterGroup (EnergyMeters(WhichMeter))
        CALL WriteMeterDictionaryItem (FrequencyIndicator, 1, EnergyMeters(WhichMeter)%TSAccRptNum, &
  ...
  CASE (1)
    IF (.not. CumulativeIndicator) THEN
      ...
      IF (.not. EnergyMeters(WhichMeter)%RptHR) THEN
        ...
        indexGroupKey = DetermineIndexGroupKeyFromMeterName (EnergyMeters(WhichMeter)%Name)
        indexGroup    = DetermineIndexGroupFromMeterGroup (EnergyMeters(WhichMeter))
        CALL WriteMeterDictionaryItem (FrequencyIndicator, 1, EnergyMeters(WhichMeter)%HRRptNum, indexGroupKey, indexGroup, &
                EnergyMeters(WhichMeter)%HRRptNumChr, EnergyMeters(WhichMeter)%Name, &
                EnergyMeters(WhichMeter)%Units, .false., MeterFileOnlyIndicator)
      ENDIF
    ELSE
      IF (MeterFileOnlyIndicator) THEN
      ...
      IF (.not. EnergyMeters(WhichMeter)%RptAccHR) THEN
        ...
        indexGroupKey = DetermineIndexGroupKeyFromMeterName (EnergyMeters(WhichMeter)%Name)
        indexGroup    = DetermineIndexGroupFromMeterGroup (EnergyMeters(WhichMeter))
        CALL WriteMeterDictionaryItem (FrequencyIndicator, 1, EnergyMeters(WhichMeter)%HRAccRptNum, indexGroupKey, indexGroup, &
                TrimSigDigits(EnergyMeters(WhichMeter)%HRAccRptNum), EnergyMeters(WhichMeter)%Name, &
                EnergyMeters(WhichMeter)%Units, .true., MeterFileOnlyIndicator)
      ENDIF
    ENDIF
  CASE (2)
    IF (.not. CumulativeIndicator) THEN
      ...
      IF (.not. EnergyMeters(WhichMeter)%RptDY) THEN
        ...
        indexGroupKey = DetermineIndexGroupKeyFromMeterName (EnergyMeters(WhichMeter)%Name)
        indexGroup    = DetermineIndexGroupFromMeterGroup (EnergyMeters(WhichMeter))
        CALL WriteMeterDictionaryItem (FrequencyIndicator, 1, EnergyMeters(WhichMeter)%DYRptNum, indexGroupKey, indexGroup, &
                EnergyMeters(WhichMeter)%DYRptNumChr, EnergyMeters(WhichMeter)%Name, &
                EnergyMeters(WhichMeter)%Units, .false., MeterFileOnlyIndicator)
      ENDIF
    ELSE
      IF (MeterFileOnlyIndicator) THEN
      ...
      IF (.not. EnergyMeters(WhichMeter)%RptAccDY) THEN
        ...
        indexGroupKey = DetermineIndexGroupKeyFromMeterName (EnergyMeters(WhichMeter)%Name)
        indexGroup    = DetermineIndexGroupFromMeterGroup (EnergyMeters(WhichMeter))
        CALL WriteMeterDictionaryItem (FrequencyIndicator, 1, EnergyMeters(WhichMeter)%DYAccRptNum, indexGroupKey, indexGroup, &
                TrimSigDigits(EnergyMeters(WhichMeter)%DYAccRptNum), EnergyMeters(WhichMeter)%Name, &
                EnergyMeters(WhichMeter)%Units, .true., MeterFileOnlyIndicator)
      ENDIF
    ENDIF
  CASE (3)
    IF (.not. CumulativeIndicator) THEN
      ...
      IF (.not. EnergyMeters(WhichMeter)%RptMN) THEN
        ...
        indexGroupKey = DetermineIndexGroupKeyFromMeterName (EnergyMeters(WhichMeter)%Name)
        indexGroup    = DetermineIndexGroupFromMeterGroup (EnergyMeters(WhichMeter))
        CALL WriteMeterDictionaryItem (FrequencyIndicator, 1, EnergyMeters(WhichMeter)%MNRptNum, indexGroupKey, indexGroup, &
                EnergyMeters(WhichMeter)%MNRptNumChr, EnergyMeters(WhichMeter)%Name, &
                EnergyMeters(WhichMeter)%Units, .false., MeterFileOnlyIndicator)
      ENDIF
    ELSE
      IF (MeterFileOnlyIndicator) THEN
      ...
      IF (.not. EnergyMeters(WhichMeter)%RptAccMN) THEN
        ...
        indexGroupKey = DetermineIndexGroupKeyFromMeterName (EnergyMeters(WhichMeter)%Name)
        indexGroup    = DetermineIndexGroupFromMeterGroup (EnergyMeters(WhichMeter))
        CALL WriteMeterDictionaryItem (FrequencyIndicator, 1, EnergyMeters(WhichMeter)%MNAccRptNum, indexGroupKey, indexGroup, &
                TrimSigDigits(EnergyMeters(WhichMeter)%MNAccRptNum), EnergyMeters(WhichMeter)%Name, &
                EnergyMeters(WhichMeter)%Units, .true., MeterFileOnlyIndicator)
      ENDIF
    ENDIF
  CASE (4)
    IF (.not. CumulativeIndicator) THEN
      ...
      IF (.not. EnergyMeters(WhichMeter)%RptSM) THEN
        ...
        indexGroupKey = DetermineIndexGroupKeyFromMeterName (EnergyMeters(WhichMeter)%Name)
        indexGroup    = DetermineIndexGroupFromMeterGroup (EnergyMeters(WhichMeter))
        CALL WriteMeterDictionaryItem (FrequencyIndicator, 1, EnergyMeters(WhichMeter)%SMRptNum, indexGroupKey, indexGroup, &
                EnergyMeters(WhichMeter)%SMRptNumChr, EnergyMeters(WhichMeter)%Name, &
                EnergyMeters(WhichMeter)%Units, .false., MeterFileOnlyIndicator)
      ENDIF
    ELSE
      IF (MeterFileOnlyIndicator) THEN
      ...
      IF (.not. EnergyMeters(WhichMeter)%RptAccSM) THEN
        ...
        indexGroupKey = DetermineIndexGroupKeyFromMeterName (EnergyMeters(WhichMeter)%Name)
        indexGroup    = DetermineIndexGroupFromMeterGroup (EnergyMeters(WhichMeter))
        CALL WriteMeterDictionaryItem (FrequencyIndicator, 1, EnergyMeters(WhichMeter)%SMAccRptNum, indexGroupKey, indexGroup, &
                TrimSigDigits(EnergyMeters(WhichMeter)%SMAccRptNum), EnergyMeters(WhichMeter)%Name, &
                EnergyMeters(WhichMeter)%Units, .true., MeterFileOnlyIndicator)
      ENDIF
    ENDIF
  CASE DEFAULT
...
SUBROUTINE GetVariableKeys(varName,varType,keyNames,keyVarIndexes)
  ...
  varNameUpper     = MakeUPPERCase(TRIM(varName))
  ...
  DO Loop=1,NumReqMeters
    ...
    If (Meter > 0) Then !All the active meters for this run are set, but all are still searched for.
      ...
      CASE (ReportTimeStep)
        ...
        Else
          EnergyMeters(Meter)%RptTS=.true.
          CALL WriteMeterDictionaryItem (ReportFreq, 1, EnergyMeters(Meter)%TSRptNum, &
      ...
      CASE (ReportHourly)
        ...
        Else
          EnergyMeters(Meter)%RptHR=.true.
          TrackingHourlyVariables=.true.
          CALL WriteMeterDictionaryItem (ReportFreq, 1, EnergyMeters(Meter)%HRRptNum, &
      ...
      CASE (ReportDaily)
        ...
        Else
          EnergyMeters(Meter)%RptDY=.true.
          TrackingDailyVariables=.true.
          CALL WriteMeterDictionaryItem (ReportFreq, 1, EnergyMeters(Meter)%DYRptNum, &
      ...
      CASE (ReportMonthly)
        ...
        Else
          EnergyMeters(Meter)%RptMN=.true.
          TrackingMonthlyVariables=.true.
          CALL WriteMeterDictionaryItem (ReportFreq, 1, EnergyMeters(Meter)%MNRptNum, &
      ...
      CASE (ReportSim)
        ...
        Else
          EnergyMeters(Meter)%RptSM=.true.
          TrackingRunPeriodVariables=.true.
          CALL WriteMeterDictionaryItem (ReportFreq, 1, EnergyMeters(Meter)%SMRptNum, &
