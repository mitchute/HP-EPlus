!
!  NOTE: This file contains 2 modules: ChillerAbsorption and ChillerIndirectAbsorption
!
...
USE DataLoopNode
USE DataInterfaces
USE DataGlobals_HPSimIntegrated,     ONLY: MaxNameLength, InitConvTemp
...
SUBROUTINE SimBLASTAbsorber(AbsorberType,AbsorberName,EquipFlowCtrl,LoopNum,LoopSide,CompIndex,RunFlag,FirstIteration, &
  ...
  IF (InitLoopEquip) THEN
    ...
    CALL InitBLASTAbsorberModel(ChillNum,RunFlag,MyLoad, FirstIteration)
...
!  CHARACTER(len=MaxNameLength) :: CurrentModuleObject  ! for ease in renaming.
  ...
  NumBLASTAbsorbers = GetNumObjectsFound(TRIM(cCurrentModuleObject))
  ...
  DO AbsorberNum = 1 , NumBLASTAbsorbers
    ...
    If(cAlphaArgs(8) .eq. "CONSTANTFLOW") Then
       BLASTAbsorber(AbsorberNum)%ConstantFlow = .True.
       BLASTAbsorber(AbsorberNum)%VariableFlow = .False.
    Else If(cAlphaArgs(8) .eq. "VARIABLEFLOW") Then
       BLASTAbsorber(AbsorberNum)%ConstantFlow = .False.
       BLASTAbsorber(AbsorberNum)%VariableFlow = .True.
    Else  ! We will assume a variable flow chiller if none specified
       BLASTAbsorber(AbsorberNum)%ConstantFlow = .False.
       BLASTAbsorber(AbsorberNum)%VariableFlow = .True.
    End If
    ...
    IF(BLASTAbsorber(AbsorberNum)%GeneratorVolFlowRate == 0.0 .AND. &
  ...
  DO AbsorberNum = 1, NumBLASTAbsorbers
     ...
     IF(BLASTAbsorber(AbsorberNum)%GenHeatSourceType == NodeType_Water)THEN
       ...
       CALL SetupOutputVariable('Chiller Hot Water Consumption [J]', &
     ...
     ELSE
       IF(GenInputOutputNodesUsed(AbsorberNum))THEN
         CALL SetupOutputVariable('Chiller Steam Consumption Rate [W]', &
              BLASTAbsorberReport(AbsorberNum)%QGenerator,'System','Average',BLASTAbsorber(AbsorberNum)%Name)
         CALL SetupOutputVariable('Chiller Steam Consumption [J]', &
              BLASTAbsorberReport(AbsorberNum)%GeneratorEnergy,'System','Sum',BLASTAbsorber(AbsorberNum)%Name,  &
                              ResourceTypeKey='PLANTLOOPHEATINGDEMAND',EndUseKey='CHILLERS',GroupKey='Plant')
       ELSE
         CALL SetupOutputVariable('Chiller Steam Consumption Rate [W]', &
...
SUBROUTINE InitBLASTAbsorberModel(ChillNum,RunFlag, MyLoad, FirstHVACIteration)
  ...
  IF (MyFlag(ChillNum)) THEN
    ...
    IF (BLASTAbsorber(ChillNum)%VariableFlow) THEN
      ...
      IF ((Node(BLASTAbsorber(ChillNum)%EvapOutletNodeNum)%TempSetPoint == SensedNodeFlagValue) .AND. &
        ...
        IF (.NOT. AnyEnergyManagementSystemInModel) THEN
          IF (.NOT. BLASTAbsorber(ChillNum)%VariableFlowErrDone) THEN
            ...
            BLASTAbsorber(ChillNum)%VariableFlowErrDone = .TRUE.
          ENDIF
        ELSE
        ...
        BLASTAbsorber(ChillNum)%VariableFlowSetToLoop = .TRUE.
...
!     .OR. (Node(CondInletNode)%MassFlowrate <= 0.0 .AND. RunFlag)) THEN
  ...
  IF (BLASTAbsorber(ChillNum)%VariableFlow .AND. BLASTAbsorber(ChillNum)%VariableFlowSetToLoop) THEN
...
!       QCondenser = QEvaporator + QGenerator + PumpingPower
    ...
    ELSE
      CALL ShowContinueError('Autosizing of Absorption Chiller condenser flow rate requires a condenser')
...
SUBROUTINE CalcBLASTAbsorberModel(ChillNum,MyLoad,Runflag,FirstIteration,EquipFlowCtrl)
          ...
          ! USE STATEMENTS:
  USE DataGlobals_HPSimIntegrated,     ONLY : BeginEnvrnFlag, SecInHour, WarmupFlag
...
!  LOGICAL,SAVE           :: PossibleSubCooling
  ...
  IF(MyLoad >= 0.0 .OR. .NOT. RunFlag) THEN !off or heating
  ...
  IF (PlantLoop(LoopNum)%Loopside(LoopSideNum)%FlowLock==0) THEN
    ...
    Else IF(BLASTAbsorber(ChillNum)%VariableFlow)Then
  ...
  ELSE  ! If FlowLock is True
    ...
    ELSE
      ...
      CASE (SingleSetpoint)
        IF ((BLASTAbsorber(ChillNum)%VariableFlow) .OR. &
...
SUBROUTINE UpdateBLASTAbsorberRecords(MyLoad,RunFlag, Num)
  ...
  IF (MyLoad>=0 .OR. .NOT. RunFlag)THEN
   !set node conditions
    ...
    BLASTAbsorberReport(Num)%PumpingPower     = 0
    BLASTAbsorberReport(Num)%QEvap            = 0
    BLASTAbsorberReport(Num)%QCond            = 0
    BLASTAbsorberReport(Num)%QGenerator       = 0
    ...
    BLASTAbsorberReport(Num)%Evapmdot         = 0
    BLASTAbsorberReport(Num)%Condmdot         = 0
    BLASTAbsorberReport(Num)%Genmdot         = 0
...
SUBROUTINE SimIndirectAbsorber(AbsorberType,AbsorberName,EquipFlowCtrl,LoopNum,LoopSide,CompIndex,RunFlag,FirstIteration, &
  ...
  ENDIF
   ...
   ! Initialize Loop Equipment
  IF (InitLoopEquip) THEN
    CALL InitIndirectAbsorpChiller(ChillNum,RunFlag, MyLoad, FirstIteration)
...
SUBROUTINE GetIndirectAbsorberInput
  ...
  NumIndirectAbsorbers = GetNumObjectsFound(TRIM(cCurrentModuleObject))
...
!     Generator fluid type must be steam if generator inlet/outlet nodes are not used
    ...
    If(cAlphaArgs(6) .eq. "CONSTANTFLOW") Then
       IndirectAbsorber(AbsorberNum)%ConstantFlow = .TRUE.
       IndirectAbsorber(AbsorberNum)%VariableFlow = .FALSE.
    Else If(cAlphaArgs(6) .eq. "VARIABLEFLOW") Then
       IndirectAbsorber(AbsorberNum)%ConstantFlow = .FALSE.
       IndirectAbsorber(AbsorberNum)%VariableFlow = .TRUE.
    Else  ! Default to a variable flow chiller if none specified
       IndirectAbsorber(AbsorberNum)%ConstantFlow = .FALSE.
       IndirectAbsorber(AbsorberNum)%VariableFlow = .TRUE.
    End If
    ...
    IF(IndirectAbsorber(AbsorberNum)%GeneratorVolFlowRate == 0.0 .AND. &
  ...
  DO AbsorberNum = 1, NumIndirectAbsorbers
     ...
     CALL SetupOutputVariable('Chiller Cycling Fraction', &
          IndirectAbsorberReport(AbsorberNum)%ChillerCyclingFrac,'System','Average',IndirectAbsorber(AbsorberNum)%Name)

     CALL SetupOutputVariable('Chiller Loop Losses[W]', IndirectAbsorberReport(AbsorberNum)%LoopLoss, &
...
SUBROUTINE InitIndirectAbsorpChiller(ChillNum,RunFlag, MyLoad, FirstHVACIteration)
  ...
  IF (MyFlag(ChillNum)) THEN
   ...
   IF ((IndirectAbsorber(ChillNum)%CondInletNodeNum > 0 ) .AND. &
    ...
    IF (IndirectAbsorber(ChillNum)%VariableFlow) THEN
      ! reset flow priority
      ...
      IF ((Node(IndirectAbsorber(ChillNum)%EvapOutletNodeNum)%TempSetPoint == SensedNodeFlagValue) .AND. &
        ...
        IF (.NOT. AnyEnergyManagementSystemInModel) THEN
          IF (.NOT. IndirectAbsorber(ChillNum)%VariableFlowErrDone) THEN
            CALL ShowWarningError('Missing temperature setpoint for VariableFlow mode chiller named ' // &
                                          TRIM(IndirectAbsorber(ChillNum)%Name) )
            CALL ShowContinueError('  A temperature setpoint is needed at the outlet node of a chiller ' // &
                                             'in variable flow mode, use a SetpointManager')
            CALL ShowContinueError('  The overall loop setpoint will be assumed for chiller. The simulation continues ... ')
            IndirectAbsorber(ChillNum)%VariableFlowErrDone = .TRUE.
          ENDIF
        ELSE
         ! need call to EMS to check node
          FatalError = .FALSE. ! but not really fatal yet, but should be.
        ...
        IndirectAbsorber(ChillNum)%VariableFlowSetToLoop = .TRUE.
  ...
  IF (IndirectAbsorber(ChillNum)%VariableFlow .AND. IndirectAbsorber(ChillNum)%VariableFlowSetToLoop) THEN
  ! fix for clumsy old input that worked because loop setpoint was spread.
  !  could be removed with transition, testing , model change, period of being obsolete.
    Node(IndirectAbsorber(ChillNum)%EvapOutletNodeNum)%TempSetPoint =                        &
         Node(PlantLoop(IndirectAbsorber(ChillNum)%CWLoopNum)%TempSetPointNodeNum)%TempSetPoint
...
! condenser side outlet temperature.
  IF (PlantLoop(LoopNum)%Loopside(LoopSideNum)%FlowLock==0) THEN
     ...
     ELSE IF(IndirectAbsorber(ChillNum)%VariableFlow)THEN
        ! Calculate the Delta Temp from the inlet temp to the chiller outlet setpoint
  ...
  ELSE  ! If FlowLock is True
     ...
     ELSE
       SELECT CASE (PlantLoop(LoopNum)%LoopDemandCalcScheme)
       CASE (SingleSetpoint)
         IF ((IndirectAbsorber(ChillNum)%VariableFlow) .OR. &
