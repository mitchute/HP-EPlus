...
USE DataLoopNode
USE DataGlobals_HPSimIntegrated ,    ONLY : MaxNameLength, NumOfTimeStepInHour, InitConvTemp, WarmupFlag
USE DataInterfaces
  ...
  DO ChillerNum = 1 , NumElectricChillers
   ...
   ! These are the Heat Recovery Inputs
    ...
    IF (ElectricChiller(ChillerNum)%DesignHeatRecVolFlowRate > 0.0) THEN
      ...
      IF (ElectricChiller(ChillerNum)%Base%CondenserType == AirCooled .OR. &
          ElectricChiller(ChillerNum)%Base%CondenserType == EvapCooled) THEN
        IF(ElectricChiller(ChillerNum)%Base%CondVolFlowRate .LE. 0.0)THEN
...
!Load Special EngineDriven Chiller Curve Fit Inputs
    ...
    IF (EngineDrivenChiller(ChillerNum)%DesignHeatRecVolFlowRate > 0.0) THEN
      ...
      IF (EngineDrivenChiller(ChillerNum)%Base%CondenserType == AirCooled .OR. &
          EngineDrivenChiller(ChillerNum)%Base%CondenserType == EvapCooled) THEN
        IF(EngineDrivenChiller(ChillerNum)%Base%CondVolFlowRate .LE. 0.0)THEN
          CALL ShowSevereError('Invalid '//TRIM(cNumericFieldNames(10))//'='//TRIM(RoundSigDigits(rNumericArgs(10),6)))
          CALL ShowSevereError('Condenser fluid flow rate must be specified for Heat Reclaim applications.')
          CALL ShowContinueError('Entered in '//TRIM(cCurrentModuleObject)//'='//TRIM(cAlphaArgs(1)))
          ErrorsFound=.true.
...
SUBROUTINE GetGTChillerInput
  ...
  DO ChillerNum = 1 , NumGTChillers
    ...
    IF (GTChiller(ChillerNum)%DesignHeatRecVolFlowRate > 0.0) THEN
...
SUBROUTINE InitElectricChiller(ChillNum,RunFlag, MyLoad, FirstHVACIteration)
          ...
          ! USE STATEMENTS:
  USE DataGlobals_HPSimIntegrated,     ONLY : BeginEnvrnFlag, AnyEnergyManagementSystemInModel !, ShowFatalError, ShowSevereError, ShowContinueError
    ...
    IF (PltSizCondNum > 0) THEN
      IF (PlantSizData(PltSizNum)%DesVolFlowRate >= SmallWaterVolFlow) THEN
        ...
        Cp = GetSpecificHeatGlycol(PlantLoop(GTChiller(ChillNum)%Base%CDLoopNum)%FluidName,  &
                                 ...
                                 PlantLoop(GTChiller(ChillNum)%Base%CDLoopNum)%FluidIndex, &
                                 'SizeGTChiller')
        tmpCondVolFlowRate = tmpNomCap *  (1. + 1./GTChiller(ChillNum)%Base%COP) / &
                                ( PlantSizData(PltSizCondNum)%DeltaT * Cp * Rho)
...
SUBROUTINE CalcElectricChillerModel(ChillNum,MyLoad,EquipFlowCtrl,Runflag,FirstIteration)
          ...
          ! USE STATEMENTS:
  USE DataPrecisionGlobals
  USE DataGlobals_HPSimIntegrated,     ONLY : BeginEnvrnFlag, SecInHour, outputfiledebug, CurrentTime
...
SUBROUTINE CalcEngineDrivenChillerModel(ChillerNum,MyLoad,Runflag,FirstIteration,EquipFlowCtrl)
          ...
          ! USE STATEMENTS:
  USE DataGlobals_HPSimIntegrated,     ONLY : BeginEnvrnFlag, SecInHour, CurrentTime
  USE DataHVACGlobals, ONLY : FirstTimeStepSysFlag, TimeStepSys, SysTimeElapsed
...
SUBROUTINE CalcConstCOPChillerModel(ChillNum,MyLoad,Runflag,FirstIteration,EquipFlowCtrl)
          ...
          ! USE STATEMENTS:
  USE DataGlobals_HPSimIntegrated, ONLY: SecInHour, CurrentTime !, ShowWarningError, ShowSevereError, &
...
IMPLICIT NONE
  ...
  IF (MyLoad >=0.d0 .OR. .NOT. RunFlag)THEN !Chiller not running
    ...
    EngineDrivenChillerReport(Num)%Base%Power            = 0
    EngineDrivenChillerReport(Num)%Base%QEvap            = 0
    EngineDrivenChillerReport(Num)%Base%QCond            = 0
    EngineDrivenChillerReport(Num)%Base%Energy           = 0
    EngineDrivenChillerReport(Num)%Base%EvapEnergy       = 0
    EngineDrivenChillerReport(Num)%Base%CondEnergy       = 0
...
IMPLICIT NONE
  ...
  IF (MyLoad >= 0.d0 .OR. .NOT. RunFlag)THEN !Chiller not running so pass inlet states to outlet states
    ...
    GTChillerReport(Num)%Base%Power            = 0
    GTChillerReport(Num)%Base%QEvap            = 0
    GTChillerReport(Num)%Base%QCond            = 0
    GTChillerReport(Num)%Base%Energy           = 0
    GTChillerReport(Num)%Base%EvapEnergy       = 0
    GTChillerReport(Num)%Base%CondEnergy       = 0
