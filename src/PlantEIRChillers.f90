...
SUBROUTINE InitElecReformEIRChiller(EIRChillNum, RunFlag, MyLoad, FirstHVACIteration)
  ...
  ! USE STATEMENTS:
  USE DataGlobals_HPSimIntegrated,     ONLY : BeginEnvrnFlag, AnyEnergyManagementSystemInModel !, ShowFatalError, &
  !ShowSevereError, ShowContinueError
  ...
  IF (MyFlag(EIRChillNum)) THEN
    ...
    SUBROUTINE ControlReformEIRChillerModel(EIRChillNum,MyLoad,Runflag,FirstIteration,EquipFlowCtrl)
      ...
      USE DataGlobals_HPSimIntegrated,     ONLY : WarmupFlag !, ShowFatalError, ShowSevereError, ShowWarningError, ShowContinueError
    ...
    SUBROUTINE CalcReformEIRChillerModel(EIRChillNum,MyLoad,Runflag,FirstIteration,EquipFlowCtrl,FalsiCondOutTemp)
      ...
      ! USE STATEMENTS:
      USE DataGlobals_HPSimIntegrated,     ONLY : WarmupFlag, CurrentTime !, ShowFatalError, ShowSevereError, ShowWarningError, ShowContinueError
      USE DataInterfaces,  ONLY : ShowContinueErrorTimeStamp, ShowRecurringWarningErrorAtEnd
      USE DataHVACGlobals, ONLY : SmallLoad, SysTimeElapsed, TimeStepSys
      USE General,         ONLY : RoundSigDigits, CreateSysTimeIntervalString
      USE CurveManager,    ONLY : CurveValue
...
SUBROUTINE CheckMinMaxCurveBoundaries(EIRChillNum, FirstIteration)
  ...
  ! USE STATEMENTS:
  USE General,         ONLY : TrimSigDigits, RoundSigDigits
  USE DataGlobals_HPSimIntegrated,     ONLY : WarmupFlag !, ShowWarningError, ShowContinueError
  USE DataInterfaces,  ONLY :ShowContinueErrorTimeStamp, ShowRecurringWarningErrorAtEnd
  USE CurveManager,    ONLY : CurveValue
