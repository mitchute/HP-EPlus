...
USE DataLoopNode
USE DataInterfaces
USE DataGlobals_HPSimIntegrated ,   ONLY : MaxNameLength, BigNumber, InitConvTemp, SecInHour
...
! Part of Type that directly corresponds with IDD definition
       ...
       REAL(r64)         :: FuelCoolRatio      =0.0 ! ratio of fuel input to cooling output
       REAL(r64)         :: FuelHeatRatio      =0.0 ! ratio of fuel input to heating output
...
TYPE ReportVars
  ...
  REAL(r64)    :: FuelUseRate        =0.0 ! instantaneous use of gas for period
  REAL(r64)    :: FuelEnergy         =0.0 ! variable to track total fuel used for a period
  REAL(r64)    :: CoolFuelUseRate    =0.0 ! instantaneous use of gas for period for cooling
  REAL(r64)    :: CoolFuelEnergy     =0.0 ! variable to track total fuel used for a period for cooling
  REAL(r64)    :: HeatFuelUseRate    =0.0 ! instantaneous use of gas for period for heating
  REAL(r64)    :: HeatFuelEnergy     =0.0 ! variable to track total fuel used for a period for heating
  ...
  REAL(r64)    :: FuelCOP            =0.0 ! reporting: cooling output/fuel input = CoolingLoad/CoolFuelUseRate
...
SUBROUTINE GetGasAbsorberInput
  ...
  NumGasAbsorbers = GetNumObjectsFound(TRIM(cCurrentModuleObject))
  ...
  DO AbsorberNum = 1 , NumGasAbsorbers
    CALL GetObjectItem(TRIM(cCurrentModuleObject),AbsorberNum,cAlphaArgs,NumAlphas, &
  ...
  DO AbsorberNum = 1, NumGasAbsorbers
    ...
    CALL SetupOutputVariable('Direct Fired Absorption Chiller Heating Rate [W]', &
         GasAbsorberReport(AbsorberNum)%HeatingLoad  ,'System','Average',ChillerName)
    CALL SetupOutputVariable('Direct Fired Absorption Chiller Heating Energy [J]', &
    ...
    CALL SetupOutputVariable('Chiller '// TRIM(GasAbsorber(AbsorberNum)%FuelType)//' Consumption Rate [W]', &
    ...
    CALL SetupOutputVariable('Chiller '// TRIM(GasAbsorber(AbsorberNum)%FuelType)//' Consumption [J]', &
         GasAbsorberReport(AbsorberNum)%FuelEnergy,'System','Sum',ChillerName)

    CALL SetupOutputVariable('Direct Fired Absorption Chiller Cooling ' &
         // TRIM(GasAbsorber(AbsorberNum)%FuelType)//' Consumption Rate [W]', &
         GasAbsorberReport(AbsorberNum)%CoolFuelUseRate  ,'System','Average',ChillerName)
    CALL SetupOutputVariable('Direct Fired Absorption Chiller Cooling ' &
         // TRIM(GasAbsorber(AbsorberNum)%FuelType)//' Consumption [J]', &
    ...
    CALL SetupOutputVariable('Direct Fired Absorption Chiller Cooling Fuel COP [W/W]', &
         GasAbsorberReport(AbsorberNum)%FuelCOP ,'System','Average',ChillerName)

    CALL SetupOutputVariable('Direct Fired Absorption Chiller Heating '// TRIM(GasAbsorber(AbsorberNum)%FuelType)//  &
         ' Consumption Rate [W]',GasAbsorberReport(AbsorberNum)%HeatFuelUseRate  ,'System','Average',ChillerName)
    CALL SetupOutputVariable('Direct Fired Absorption Chiller Heating '// TRIM(GasAbsorber(AbsorberNum)%FuelType)//  &
         ' Consumption [J]',GasAbsorberReport(AbsorberNum)%HeatFuelEnergy,'System','Sum',ChillerName, &
    ...
    CALL SetupOutputVariable('Direct Fired Absorption Chiller Cooling Electric Power [W]', &
         GasAbsorberReport(AbsorberNum)%CoolElectricPower  ,'System','Average',ChillerName)
    CALL SetupOutputVariable('Direct Fired Absorption Chiller Cooling Electric Consumption [J]', &
    ...
    CALL SetupOutputVariable('Direct Fired Absorption Chiller Heating Electric Power [W]', &
         GasAbsorberReport(AbsorberNum)%HeatElectricPower  ,'System','Average',ChillerName)
    CALL SetupOutputVariable('Direct Fired Absorption Chiller Heating Electric Consumption [J]', &
    ...
    CALL SetupOutputVariable('Direct Fired Absorption Chiller Hot Water Inlet Temp [C]', &
         GasAbsorberReport(AbsorberNum)%HotWaterReturnTemp  ,'System','Average',ChillerName)
    CALL SetupOutputVariable('Direct Fired Absorption Chiller Hot Water Outlet Temp [C]', &
         GasAbsorberReport(AbsorberNum)%HotWaterSupplyTemp  ,'System','Average',ChillerName)
    CALL SetupOutputVariable('Direct Fired Absorption Chiller Hot Water Mass Flow Rate [kg/s]', &
         GasAbsorberReport(AbsorberNum)%HotWaterFlowRate  ,'System','Average',ChillerName)

    CALL SetupOutputVariable('Direct Fired Absorption Chiller Cooling Part Load Ratio', &
         GasAbsorberReport(AbsorberNum)%CoolPartLoadRatio  ,'System','Average',ChillerName)
    CALL SetupOutputVariable('Direct Fired Absorption Chiller Cooling Capacity [W]', &
         GasAbsorberReport(AbsorberNum)%CoolingCapacity  ,'System','Average',ChillerName)
    CALL SetupOutputVariable('Direct Fired Absorption Chiller Heating Part Load Ratio', &
         GasAbsorberReport(AbsorberNum)%HeatPartLoadRatio  ,'System','Average',ChillerName)
    CALL SetupOutputVariable('Direct Fired Absorption Chiller Heating Capacity [W]', &
         GasAbsorberReport(AbsorberNum)%HeatingCapacity  ,'System','Average',ChillerName)

    CALL SetupOutputVariable('Direct Fired Absorption Chiller Fraction of Period Running', &
...
SUBROUTINE InitGasAbsorber(ChillNum,RunFlag)
          ...
          ! USE STATEMENTS:
  USE DataGlobals_HPSimIntegrated,     ONLY : BeginEnvrnFlag, AnyEnergyManagementSystemInModel
...
!  LOGICAL             :: LoopErrorsFound
  ...
  IF (GasAbsorber(ChillNum)%NomCoolingCap  == AutoSize) THEN
    ...
    ELSE
      ...
      CALL ShowContinueError('Occurs in Direct Fired Absorption Chiller object='//TRIM(GasAbsorber(ChillNum)%Name))
      ErrorsFound = .TRUE.
    END IF
  END IF
  ...
  IF ((GasAbsorber(ChillNum)%CondVolFlowRate == AutoSize) .AND. (GasAbsorber(ChillNum)%isWaterCooled)) THEN
    IF (PltSizCondNum > 0) THEN
      IF (PlantSizData(PltSizCoolNum)%DesVolFlowRate >= SmallWaterVolFlow) THEN
        ...
        tmpCondVolFlowRate = tmpNomCap * &
                             (1. + GasAbsorber(ChillNum)%FuelCoolRatio) / &
    ...
    ELSE
      CALL ShowSevereError('Autosizing of Direct Fired Absorption Chiller condenser flow rate requires a condenser')
...
SUBROUTINE CalcGasAbsorberChillerModel(ChillNum,MyLoad,Runflag)
          ...
          ! USE STATEMENTS:
  USE DataGlobals_HPSimIntegrated,     ONLY : BeginFullSimFlag
...
! initialize entering conditions
 ...
 ELSE
  ...
  IF(lAvailableCoolingCapacity > 0.0) THEN
...
!    Node(lCondSupplyNodeNum)%MassFlowRate           = GasAbsorberReport(ChillNum)%CondWaterFlowRate
  ...
  IF (GasAbsorberReport(ChillNum)%CoolFuelUseRate .NE. 0.0) THEN
