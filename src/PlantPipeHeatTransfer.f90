...
CONTAINS
  ...
  SUBROUTINE GetPipesHeatTransfer
    ...
    ! USE STATEMENTS:
    USE DataGlobals_HPSimIntegrated,           ONLY : NumOfZones,SecInHour,PI
  ...
  SUBROUTINE InitPipesHeatTransfer(PipeType,PipeHTNum,FirstHVACIteration)
    ...
    USE DataGlobals_HPSimIntegrated,       ONLY : BeginSimFlag, BeginEnvrnFlag, PI, DayOfSim, HourOfDay,TimeStep, &
...
Subroutine CalcBuriedPipeSoil(PipeHTNum)
  ...
  DifSolarRad, SOLCOS
  USE DataGlobals_HPSimIntegrated,            ONLY : PI, WarmupFlag, TimeStep, HourOfDay, KelvinConv, rTinyValue
