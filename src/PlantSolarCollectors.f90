MODULE SolarCollectors
  ...
  USE DataGlobals_HPSimIntegrated, ONLY: MaxNameLength, BeginEnvrnFlag !, ShowWarningError, ShowFatalError, ShowSevereError, ShowContinueError
...
CONTAINS
  ...
  SUBROUTINE GetSolarCollectorInput
    ...
    ! USE STATEMENTS:
    USE DataGlobals_HPSimIntegrated, ONLY: DegToRadians, InitConvTemp
    USE DataHeatBalance
    ...
    NumOfFlatPlateParam = GetNumObjectsFound(TRIM(CurrentModuleParamObject))
    ...
    NumFlatPlateUnits = GetNumObjectsFound(TRIM(CurrentModuleObject))
    ...
    NumOfICSParam = GetNumObjectsFound(TRIM(CurrentModuleParamObject))
    CALL GetObjectDefMaxArgs(TRIM(CurrentModuleParamObject),NumFields,NumAlphas,NumNumbers)
    MaxNumbers=MAX(MaxNumbers,NumNumbers)
    MaxAlphas=MAX(MaxAlphas,NumAlphas)

    CurrentModuleObject = 'SolarCollector:IntegralCollectorStorage'
    NumOfICSUnits = GetNumObjectsFound(TRIM(CurrentModuleObject))
    CALL GetObjectDefMaxArgs(TRIM(CurrentModuleObject),NumFields,NumAlphas,NumNumbers)
    MaxNumbers=MAX(MaxNumbers,NumNumbers)
    MaxAlphas=MAX(MaxAlphas,NumAlphas)
    ...
    DO ICSUnitsNum = 1, NumOfICSUnits
      ...
      CALL ShowContinueError('Always gross area of the collector is used in the calculation.  Modify the surface ')
    ...
    ! Setup report variables
    CALL SetupOutputVariable('ICS Solar Collector Transmittance-Absorptance Product []', &
    Collector(CollectorNum)%TauAlpha, 'System','Average', &
    Collector(CollectorNum)%Name)
    CALL SetupOutputVariable('ICS Overall Top Heat Loss Coefficient [W/m2C]', &
    Collector(CollectorNum)%UTopLoss,'System','Average', &
    Collector(CollectorNum)%Name)
    CALL SetupOutputVariable('ICS Solar Collector Absorber Plate Temperature [C]', &
    Collector(CollectorNum)%TempOfAbsPlate, 'System','Average', &
    Collector(CollectorNum)%Name)
    CALL SetupOutputVariable('ICS Solar Collector Stored Water Temperature [C]', &
    Collector(CollectorNum)%TempOfWater, 'System','Average', &
    Collector(CollectorNum)%Name)
    CALL SetupOutputVariable('ICS Solar Collector Efficiency []', &
    Collector(CollectorNum)%Efficiency, 'System','Average', &
    Collector(CollectorNum)%Name)
    CALL SetupOutputVariable('ICS Solar Collector Stored Heat Rate [W]', &
    Collector(CollectorNum)%StoredHeatRate, 'System','Average', &
    Collector(CollectorNum)%Name)
    CALL SetupOutputVariable('ICS Solar Collector Stored Heat Energy [J]', &
    ...
    CALL SetupOutputVariable('ICS Solar Collector Skin Heat Loss Rate [W]', &
    Collector(CollectorNum)%SkinHeatLossRate, &
    'System','Average',Collector(CollectorNum)%Name)
    CALL SetupOutputVariable('ICS Solar Collector Skin Heat Loss Energy [J]', &
    Collector(CollectorNum)%CollHeatLossEnergy,'System','Sum', &
    Collector(CollectorNum)%Name,ResourceTypeKey='SolarWater', &
    EndUseKey='HeatProduced',GroupKey='Plant')
    CALL SetupOutputVariable('ICS Solar Collector Heat Transfer Rate [W]', &
    Collector(CollectorNum)%HeatRate, 'System','Average', &
    Collector(CollectorNum)%Name)
    CALL SetupOutputVariable('ICS Solar Collector Heat Transfer Energy [J]', &
...
SUBROUTINE InitSolarCollector(CollectorNum, FirstHVACIteration)
  ...
  ! USE STATEMENTS:
  USE DataGlobals_HPSimIntegrated,      ONLY: SysSizingCalc, InitConvTemp, DegToRadians, TimeStepZone, TimeStep, SecInHour, &
...
SUBROUTINE CalcSolarCollector(CollectorNum)
  ...
  ! USE STATEMENTS:
  USE DataGlobals_HPSimIntegrated,     ONLY: DegToRadians
...
SUBROUTINE CalcICSSolarCollector(ColleNum)
  ...
  ! USE STATEMENTS:
  USE DataGlobals_HPSimIntegrated,     ONLY: DegToRadians, TimeStepZone, TimeStep, SecInHour, WarmupFlag, HourOfDay
  USE DataHVACGlobals, ONLY: SysTimeElapsed, TimeStepSys
...
SUBROUTINE CalcHeatTransCoeffAndCoverTemp(ColleNum)
  ...
  ! Wiley-Interscience: New York (1991).
  !
  USE DataGlobals_HPSimIntegrated,     ONLY: StefanBoltzmann, KelvinConv
...
FUNCTION CalcConvCoeffAbsPlateAndWater(TAbsorber,TWater,Lc,TiltR2V) RESULT (hConvA2W)
  ...
  !
  ! USE STATEMENTS:
  USE DataGlobals_HPSimIntegrated,     ONLY: DegToRadians, StefanBoltzmann, KelvinConv
