MODULE HighTempRadiantSystem
  ...
  USE DataGlobals_HPSimIntegrated,       ONLY : MaxNameLength, BeginTimeStepFlag, SysSizingCalc !, ShowWarningError, ShowSevereError, &
  !ShowFatalError, ShowContinueError
  USE DataInterfaces,    ONLY : SetupOutputVariable, CalcHeatBalanceOutsideSurf, CalcHeatBalanceInsideSurf
...
CONTAINS
  ...
  SUBROUTINE InitHighTempRadiantSystem(FirstHVACIteration,RadSysNum)
    ...
    ! USE STATEMENTS:
    USE DataGlobals_HPSimIntegrated,       ONLY : NumOfZones, BeginEnvrnFlag
  ...
  SUBROUTINE UpdateHighTempRadiantSystem(RadSysNum,LoadMet)
    ...
    ! USE STATEMENTS:
    USE DataGlobals_HPSimIntegrated,       ONLY : TimeStepZone,BeginEnvrnFlag
  ...
  SUBROUTINE DistributeHTRadGains
    ...
    ! USE STATEMENTS:
    USE DataGlobals_HPSimIntegrated,       ONLY : NumOfZones !, ShowSevereError, ShowFatalError, &
    !ShowContinueError, ShowWarningError
    USE DataHeatBalance,   ONLY : Zone
  ...
  SUBROUTINE ReportHighTempRadiantSystem(RadSysNum)
    ...
    ! USE STATEMENTS:
    USE DataGlobals_HPSimIntegrated,     ONLY : SecInHour,outputfiledebug
