...
USE DataGlobals_HPSimIntegrated  !includes LOGICAL :: BeginTimeStepFlag =.false.
...
SUBROUTINE ManageRefrigeratedCaseRacks
  ...
  !  waste time going through init etc. before bouncing back.  Therefore...
  IF((.NOT. HaveCasesOrWalkins) .AND. (.NOT. UseSysTimeStep)) RETURN
...
SUBROUTINE GetRefrigerationInput
  ...
  USE DataGlobals_HPSimIntegrated,       ONLY: AnyEnergyManagementSystemInModel, RefrigIndex    !RS: Debugging: Removal of plethora of RefrigIndex definitions in the code
...
!     RefrigCase(CaseNum)%DefrostType = DefElectricOnDemand
    ...
    IF((DefType==DefHotFluid   .OR. DefType==DefHotFluidTerm   .OR. &
        ...
        CALL ShowSevereError(RoutineName//TRIM(CurrentModuleObject)//'="'//TRIM(RefrigCase(CaseNum)%Name)//&
                           '", '//TRIM(cNumericFieldNames(17))//' must be greater than 0 W/m'//' for '//&
...
NStart = NumWINumberFieldsBeforeZoneInput +1
    DO ZoneID = 1,NumZones
      ...
      !The Zone Node is needed to get the zone's ambient conditions, NumOfZones from DataGlobals_HPSimIntegrated
...
END IF  !(NumTransRefrigSystems > 0)
 ...
 !    all HVAC RA fractions and write error message if greater than 1 for any zone
  DO ZoneIndex = 1,NumOfZones  !numofzones from DataGlobals_HPSimIntegrated
  ...
  IF (MyReferPlantScanFlag .AND. ALLOCATED(PlantLoop)) THEN
    DO RefCondLoop = 1, NumRefrigCondensers
      ...
      CALL ScanPlantLoopsForObject(  Condenser(RefCondLoop)%Name, &
                                     ...
                                     errFlag = errFlag )
      IF (errFlag) THEN
         CALL ShowFatalError('InitRefrigeration: Program terminated due to previous condition(s).')
      ENDIF
...
! Set total rack heat rejection used for heat reclaim. Do not allow heat reclaim on stand alone (indoor) display cases.
  ...
  IF (RefrigRack(RackNum)%EvapWaterSupplyMode == WaterSupplyFromTank) THEN
    WaterStorage(RefrigRack(RackNum)%EvapWaterSupTankID)%VdotRequestDemand(RefrigRack(RackNum)%EvapWaterTankDemandARRID) &
...
SUBROUTINE SimAirChillerSet(AirChillerSetName, ZoneNum,  FirstHVACIteration,  &
  ...
  IF(FirstHVACIteration) THEN
    DO ChillerSetID = 1,NumRefrigChillerSets
...
SUBROUTINE FinalRateCoils(Derate,SystemSourceType,SystemID,InitialTotalLoad,AvailableTotalLoad)!or unmet load and available load?
  ...
  REAL(r64), INTENT(IN)   :: AvailableTotalLoad  !Load that system or secondary loop is able to serve [W]
          ...
          ! SUBROUTINE LOCAL VARIABLE DECLARATIONS:
INTEGER      :: NumCoils             =0   ! Number of coils on this system or seconday loop
INTEGER      :: CoilID               =0   ! Index to coil
INTEGER      :: CoilIndex            =0   ! rank of coils within system
...
!    IF ( .NOT. WarmupFlag)THEN
!      Write(OutputFileDebug,708)WarmupFlag, Month, CurrentTime,AvailableTotalLoad,InitialTotalLoad,&
!                                System(SystemID)%Name
!     END IF !warmup flag print
708 format('WarmupFlag: ',A10,' Month:',1x,I2,' CurrentTime:',1x,F5.2,'  Avail Load: ',1x,F10.1,&
          ' Init Load ',1x,F10.1,'  Sys Name: ',A40)
