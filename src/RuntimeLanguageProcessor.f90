...
USE DataGlobals_HPSimIntegrated, ONLY: MaxNameLength, OutputFileDebug
...
SUBROUTINE InitializeRuntimeLanguage
          ...
          ! USE STATEMENTS:
  USE DataGlobals_HPSimIntegrated, ONLY: Pi, HourOfDay, OutputFileDebug, CurrentTime,TimeStepZone
    ...
    SELECT CASE (ErlStack(StackNum)%Instruction(InstructionNum)%Keyword)
      ...
      CASE (KeywordIf, KeywordElse)  ! same???
        ...
        IF (ExpressionNum > 0) THEN   ! could be 0 if this was an ELSE
          ...
          CALL WriteTrace(StackNum, InstructionNum, ReturnValue)
          IF (ReturnValue%Number == 0.0) THEN  !  This is the FALSE case
      ...
      CASE (KeywordEndWhile)
        ...
        InstructionNum2 =ErlStack(StackNum)%Instruction(InstructionNum)%Argument2
        ReturnValue = EvaluateExpression(ExpressionNum)
        IF ((ReturnValue%Number /= 0.0) .AND. (WhileLoopExitCounter <= MaxWhileLoopIterations)) THEN  !  This is the True case
             ! Eventually should handle strings and arrays too
...
!  Error = 'No errors.'
  ...
  DO WHILE (Pos <= LastPos)
    ...
    IF (CountDoLooping > MaxDoLoopCounts) THEN
      CALL ShowFatalError('EMS Language Processor: failed to process string ='//Trim(String) )
    ENDIF
...
!  USE DataGlobals_HPSimIntegrated, ONLY: ShowSevereError, ShowFatalError, ShowContinueError
...
RECURSIVE FUNCTION EvaluateExpression(ExpressionNum) RESULT(ReturnValue)
  ...
  USE DataGlobals_HPSimIntegrated, ONLY: DegToRadians !, ShowFatalError, ShowSevereError, ShowWarningError !unused, TimeStepZone
  ...
  ! Reduce operands down to literals
    ...
    SELECT CASE (ErlExpression(ExpressionNum)%Operator)
      ...
      CASE (FuncRandSeed)
        ...
        seedChar = RoundSigDigits(Operand(1)%Number, 0)
        SeedN    = LEN_TRIM(seedChar)
        ALLOCATE(SeedIntARR(SeedN))
        SeedIntARR = 0 ! init
        DO Pos = 1, SeedN
          SeedElementChar =  seedChar(Pos:Pos)
          READ(SeedElementChar, '(I1)')  SeedElementInt
          SeedIntARR(Pos) = SeedElementInt
        ENDDO
...
SUBROUTINE GetRuntimeLanguageUserInput
          ...
          ! USE STATEMENTS:
  USE DataGlobals_HPSimIntegrated,    ONLY: MaxNameLength, TimeStepZone !, ShowSevereError, ShowWarningError, ShowFatalError, ShowContinueError
...
!unused0909  CHARACTER(len=MaxNameLength) :: VariableName
  ...
  lNumericFieldBlanks=.false.
    ...
    IF (NumUserGlobalVariables + NumExternalInterfaceGlobalVariables > 0) THEN
      DO GlobalNum = 1, NumUserGlobalVariables + NumExternalInterfaceGlobalVariables
         ...
         IF ( GlobalNum <= NumUserGlobalVariables ) THEN
            CALL GetObjectItem(TRIM(cCurrentModuleObject), GlobalNum, cAlphaArgs, NumAlphas, rNumericArgs, &
                 NumNums, IOSTAT, AlphaBlank=lAlphaFieldBlanks, NumBlank=lNumericFieldBlanks, &
                 AlphaFieldnames=cAlphaFieldNames,NumericFieldNames=cNumericFieldNames)
         ELSE
            cCurrentModuleObject = 'ExternalInterface:Variable'
            CALL GetObjectItem(TRIM(cCurrentModuleObject), GlobalNum-NumUserGlobalVariables, cAlphaArgs, NumAlphas, rNumericArgs,&
                 NumNums, IOSTAT, AlphaBlank=lAlphaFieldBlanks, NumBlank=lNumericFieldBlanks, &
                 AlphaFieldnames=cAlphaFieldNames,NumericFieldNames=cNumericFieldNames)
         END IF
        ...
        DO ErlVarLoop = 1, NumAlphas
          ...
          IF (lAlphaFieldBlanks(ErlVarLoop)) THEN
            CALL ShowWarningError('Blank '//TRIM(cAlphaFieldNames(1))//' entered in '//TRIM(cCurrentModuleObject))
            CALL ShowContinueError('Blank entry for Erl variable will be skipped, and the simulation continues')
          ELSEIF (.not. errFlag) THEN
            ...
            IF (VariableNum > 0) THEN
              CALL ShowSevereError('Invalid '//TRIM(cAlphaFieldNames(ErlVarLoop))//'='//TRIM(cAlphaArgs(ErlVarLoop)))
              CALL ShowContinueError('Entered in '//TRIM(cCurrentModuleObject))
    ...
    IF (NumEMSCurveIndices > 0) THEN
      ...
      DO Loop =1, NumEMSCurveIndices
        CALL GetObjectItem(TRIM(cCurrentModuleObject), Loop, cAlphaArgs, NumAlphas, rNumericArgs, &
             NumNums, IOSTAT, AlphaBlank=lAlphaFieldBlanks, NumBlank=lNumericFieldBlanks, &
             AlphaFieldnames=cAlphaFieldNames,NumericFieldNames=cNumericFieldNames)
        ...
        CALL ValidateEMSVariableName(cCurrentModuleObject,cAlphaArgs(1),cAlphaFieldNames(1),  &
          ...
          IF (lAlphaFieldBlanks(1)) THEN
            CALL ShowSevereError('Blank '//TRIM(cAlphaFieldNames(1))//' entered in '//TRIM(cCurrentModuleObject))
          ...
          IF (CurveIndexNum == 0) THEN
            IF (lAlphaFieldBlanks(2)) THEN
              CALL ShowSevereError('Blank '//TRIM(cAlphaFieldNames(2))//' entered in ' &
    ...
    IF (NumErlPrograms > 0) THEN
      ...
      DO StackNum = 1, NumErlPrograms
        CALL GetObjectItem(TRIM(cCurrentModuleObject), StackNum, cAlphaArgs, NumAlphas, rNumericArgs, &
                      NumNums, IOSTAT, AlphaBlank=lAlphaFieldBlanks, NumBlank=lNumericFieldBlanks, &
                      AlphaFieldnames=cAlphaFieldNames,NumericFieldNames=cNumericFieldNames)
        IsNotOK = .FALSE.
        IsBlank = .FALSE.
    ...
    IF (NumErlSubroutines > 0) THEN
      ...
      DO StackNum = NumErlPrograms + 1, NumErlStacks
        CALL GetObjectItem(TRIM(cCurrentModuleObject), StackNum - NumErlPrograms, cAlphaArgs, NumAlphas, rNumericArgs, &
                      NumNums, IOSTAT, AlphaBlank=lAlphaFieldBlanks, NumBlank=lNumericFieldBlanks, &
                      AlphaFieldnames=cAlphaFieldNames,NumericFieldNames=cNumericFieldNames)
    ...
    NumErlTrendVariables = GetNumObjectsFound(TRIM(cCurrentModuleObject))
    IF (NumErlTrendVariables > 0) THEN
      ALLOCATE (TrendVariable(NumErlTrendVariables))
      DO TrendNum = 1, NumErlTrendVariables
        CALL GetObjectItem(TRIM(cCurrentModuleObject), TrendNum, cAlphaArgs, NumAlphas, rNumericArgs, &
                      NumNums, IOSTAT, AlphaBlank=lAlphaFieldBlanks, NumBlank=lNumericFieldBlanks, &
                      AlphaFieldnames=cAlphaFieldNames,NumericFieldNames=cNumericFieldNames)
        IsNotOK = .FALSE.
        IsBlank = .FALSE.
        ...
        IF (NumTrendSteps > 0) THEN
          ...
          ENDDO
        ELSE
          CALL ShowSevereError('Invalid '//Trim(cNumericFieldNames(1))//'='//TRIM(TrimSigDigits(rNumericArgs(1),2)))
          CALL ShowContinueError('Entered in '//TRIM(cCurrentModuleObject)//'='//TRIM(cAlphaArgs(1)))
    ...
    IF (NumEMSOutputVariables > 0) THEN
      ...
      DO RuntimeReportVarNum = 1, NumEMSOutputVariables
        CALL GetObjectItem(TRIM(cCurrentModuleObject), RuntimeReportVarNum, cAlphaArgs, NumAlphas, rNumericArgs, &
                      NumNums, IOSTAT, AlphaBlank=lAlphaFieldBlanks, NumBlank=lNumericFieldBlanks, &
                      AlphaFieldnames=cAlphaFieldNames,NumericFieldNames=cNumericFieldNames)
