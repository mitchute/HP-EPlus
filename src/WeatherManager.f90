MODULE WeatherManager          ! EnergyPus Simulation Module
  ...
  TYPE EnvironmentData
    ...
    INTEGER :: KindOfEnvrn                = 0       ! Type of environment (see Parameters for KindOfSim in DataGlobals_HPSimIntegrated)
...
CONTAINS
  ...
  SUBROUTINE GetNextEnvironment(Available,ErrorsFound)
    ...
    ELSE
      ...
      NumOfDayInEnvrn = Environment(Envrn)%TotalDays  ! Set day loop maximum from DataGlobals_HPSimIntegrated
  ...
  ! set the EndDesignDayEnvrnsFlag (DataGlobals_HPSimIntegrated)
...
SUBROUTINE SetCurrentWeather
  ...
  !RS: Debugging: Operation Constant Outdoor Conditions (3/10/15) Mission: To Determine Where the Load Wiggle Comes From
  !OutDryBulbTemp=34 !31 !23
  !OutRelHum=34 !49 !57
  !OutDewPointTemp=19.03
...
SUBROUTINE GetTempsOut(Out1, Out2, Out3, Out4) !, Out5, Out6)
  ...
  !Out5=OutHumRat  !Outdoor Humidity Ratio
  !Out6=OutAirDensity !Outdoor Air Density
