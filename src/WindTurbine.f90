...
USE DataGenerators
USE DataGlobals_HPSimIntegrated,   ONLY : MaxNameLength, Pi, SecInHour, BeginEnvrnFlag, DegToRadians !, ShowWarningError, ShowSevereError, ShowFatalError, ShowContinueError
...
DO WindTurbineNum = 1, NumWindTurbines
  ...
  CALL GetObjectItem(CurrentModuleObject,WindTurbineNum,cAlphaArgs,NumAlphas,rNumericArgs,NumNumbers,IOStat,  &
    ...
    IF (WindTurbineSys(WindTurbineNum)%RatedWindSpeed == 0.0) THEN
    ...
    IF (WindTurbineSys(WindTurbineNum)%CutInSpeed == 0.0) THEN
    ...
    IF (WindTurbineSys(WindTurbineNum)%CutOutSpeed == 0.0) THEN
