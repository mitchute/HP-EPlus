MODULE ZonePlenum
  ...
  USE DataGlobals_HPSimIntegrated, ONLY: BeginEnvrnFlag,BeginDayFlag, NumOfZones !, ShowSevereError,ShowFatalError, ShowContinueError
  USE DataLoopNode
  USE DataHVACGlobals
...
CONTAINS
  ...
  SUBROUTINE SimAirZonePlenum(CompName, iCompType, CompIndex, FirstHVACIteration, FirstCall, PlenumInletChanged)
    ...
    LOGICAL, OPTIONAL, INTENT(IN) :: FirstHVACIteration
  ...
  SUBROUTINE GetZonePlenumInput
    ...
    DO ZonePlenumLoop = 1, NumZoneReturnPlenums
      ...
      CALL GetObjectItem(TRIM(CurrentModuleObject),ZonePlenumNum,AlphArray,NumAlphas,NumArray,NumNums,IOSTAT, &
  ...
  SUBROUTINE InitAirZoneSupplyPlenum(ZonePlenumNum, FirstHVACIteration, FirstCall)
    ...
    IF (FirstCall) THEN
      ...
      IF(Node(InletNode)%MassFlowRateMaxAvail == 0.0) THEN !For Node inlet Max Avail = 0.0
  ...
  SUBROUTINE CalcAirZoneReturnPlenum(ZonePlenumNum)
    ...
    IF (ZoneRetPlenCond(ZonePlenumNum)%OutletMassFlowRate .GT. 0.0) THEN
